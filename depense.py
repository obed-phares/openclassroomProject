...
class categorie:
    def __init__(self, root):
        ...
        cur=con.cursor()
        cur.execute("CREATE TABLE IF NOT EXISTS depense(pid INTEGER PRIMARY KEY AUTOINCREMENT, categorie text, fournisseur text, nom text, prix text, quantite text, status text)")
        con.commit()
        ...
        titre =Label(produit_frame, text="Détail depense", font=("times new roman", 15), bg="cyan").pack(side=TOP, fill=X)
        ...
        lbl_nom=Label(produit_frame, text="nom", font=("times new roman ", 13), bg="white").place(x=150, y=30)
        txt_nom=Entry(produit_frame,  font=("times new roman", 13), bg="lightyellow").place(x=150, y=50)
        ...
        lbl_nom=Label(produit_frame, text="nature depense", font=("times new roman ", 13), bg="white").place(x=150, y=80)
        txt_nom=Entry(produit_frame,  font=("times new roman", 13), bg="lightyellow").place(x=150, y=100)

        cal1=Label(produit_frame, text="Date", font=("times new roman ", 13), bg="white").place(x=150, y=120)
        self.cal1=DateEntry(self.root,selectmode='day')
        self.cal1.place(x=150, y=180,width=150)


        #cal2=Label(produit_frame, text="Au", font=("times new roman ", 15), bg="white").place(x=150, y=180)
        #self.cal2=DateEntry(self.root,selectmode='day')
        #self.cal2.place(x=150, y=210,width=200)

        lbl_prix=Label(produit_frame, text="Coût", font=("times new roman ", 15), bg="white").place(x=5, y=180)
        ...
        lbl_quantité=Label(produit_frame, text="Détail", font=("times new roman ", 15), bg="white").place(x=5, y=230)
        txt_quantité=Entry(produit_frame, textvariable=self.var_quantite, font=("times new roman", 15), bg="lightyellow").place(x=150, y=230, width=220, height=80)
        ...
        self.reini_btn= Button(produit_frame, text="renitialiser", font=("times new roman", 13), cursor="hand2", bg="gray", bd=3)
    ...
    def ajouter(self):
        ...
        try:
            if self.var_cat.get()=="selection":
                messagebox.showerror("Erreur", "veillez selection un categorie")
            else:
    ...
    def recherche(self):
        ...
        try:
             if self.var_recherche_txt.get()=="":
                ...
                cur.execute("select * from produi where" +self.var_recherche_txt.get()+  "LIKE '%'" +self.var_recherche_txt.get()+"'%")
                rows=cur.fetchone()
                if len(rows)!=0:
                    self.produitliste.delete(*self.produitliste.get_children())
                    for row in rows:
